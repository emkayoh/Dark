...
The following plots explain the parameters of the model used to predict thresholds following a photo-bleach in excess of 30% of the retinal photopigment. 
This data was extracted from Figure 1 in Pugh 1975. 
...
The initial phase depicts the cone response, and can be represented by three parameters; the absolute cone threshold ($\theta_1$, log$_{10}$(cd.m$^{\text{-}2}$) scotopic), the cone coefficient  ($\theta_2$, log$_{10}$(cd.m$^{\text{-}2}$) scotopic), and the time constant of cone recovery, sometimes known as tau ($\theta_3$, min).
...
After the cone mediated phase there follows the rod phase, S2, which is defined by its slope, 

($\theta_4$,~log$_{10}$(cd.m$^{\text{-}2}$).min$^{\text{-}1}$) 

and the rod cone transition time or alpha point ($\theta_5$, min). 

The linear (on a log-lin plot) rod phase S2, described in detail in Lamb and Pugh 2004, represents the rate limited restoration of photopigment.
...
The model uses the transition times to control step functions and hence allow each phase to combine.
...
$$
where $h(t,\theta_i)$ is a step function
$$
h(t,\theta_i)
