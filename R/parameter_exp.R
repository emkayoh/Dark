...
Ani<- function(tt=1, animate) if(animate) Sys.sleep(tt)

load('data/Pugh_1975_Dark.Rdata')

par(las=1, bty='n', mfrow=c(1,1), font=2,font.lab=2, cex.lab=1.5, oma=c(1,1,1,1), mar=c(6,6,2,3))
...
rect(Be,min(tmp$thrs),max(tmp$time),P7c(tmp$opt, Be), border=NA, col=rgb(1,0,1,0.2))
...
arrows(Al-3,P7c(tmp$opt, Al)-1.4,Al,P7c(tmp$opt, Al),  length = 0.125, angle = 20, lty=Lty)
